<?php
    ...
    if(isset($_POST['submit'])) { //กดปุ่ม เปลี่ยนรหัสผ่าน
        ...
        $sql = "UPDATE user SET password = '$newpassword' WHERE id = $id and password = '$oldpassword'"; // เตรียมคำสั่ง SQL
        $query = mysqli_query($dbcon, $sql); // สั่งให้หาข้อมูลในฐานข้อมูล ข้อมูลที่ได้จะเป็นแบบ [Array]
        
        if(mysqli_affected_rows($dbcon) > 0) { // เช็คว่ารหัสผ่านเปลี่ยนสำเร็จไหม ถ้าค่าเป็น 0 = ไม่พบข้อมูลที่ตรงเงื่อนไข WHERE, -1 = เชื่อมต่อไม่ได้, >0 = สำเร็จ
    ...
    <div class="container">
        <div class="title">เปลี่ยนรหัสผ่าน</div>
            ...
            <form  method="POST">
                ...
                <div class="input-box">
                    <span class="details">รหัสผ่านเก่า</span>
                    <input type="password" placeholder="รหัสผ่านเก่า" required name="oldpassword">
                </div>
                <div class="input-box">
                    <span class="details">รหัสผ่านใหม่</span>
                    <input type="password" placeholder="รหัสผ่านใหม่" required name="newpassword">
                </div>
                </div>
            <button class="button" type="submit" name="submit">เปลี่ยน</button>
            </form>
